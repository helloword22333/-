        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for=" item in skuAttrValueList" :key="item.id">
                <dt class="title">{{ item.saleAttrName }}</dt>
                <dd v-for="item2 in item.spuSaleAttrValueList" changepirce="0" 
                @click="changeActive(item2,item.spuSaleAttrValueList)"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" type="number" v-model.number="num" >
...
import { mapGetters } from 'vuex' 
  ...
  export default {
    ...
    mounted(){
      //这里博主没有设置占位符和传params，在search组件跳转时传参
    ...
    computed:{
      ...mapGetters(["categoryView","skuInfo","skuAttrValueList"]),
      num:{
        set(value){
          this.count=Math.abs(parseInt(value))
        },
        get(){
          return  this.count
        }
    ...
    methods:{
      ...
      async addShopCar(){
      try {
        let result=await this.$store.dispatch("addOrUpdataShopCart",{skuId:this.$route.params.skuid,skuNum:this.count})
        sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
        this.$router.push({path:"/addCarSuccess",query:{skuNum:this.count}})
      } catch (error) {
        console.log("error.")
      }
      }
